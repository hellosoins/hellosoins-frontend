...
const Formation = () => {
  ...
  const handleDeleteGroup = (entries) => {
    setDialogMessage(
      "Êtes-vous sûr de vouloir supprimer toutes ces formations ?"
    );
    setDialogAction(() => () => {
      ...
      {isEditing ? (
        ...
        <>
                    ...
                    <div className="flex gap-2">
                      <div className="w-32">
                        <Dropdown
                          ...
                          onChange={(month) => {
                            const year =
                              newExperienceDate?.split("-")[0] || currentYear;
                    ...
                    <button
                      onClick={() => {
                        setNewExperienceDate(
                          experienceData?.data?.start_date || ""
                        );
          ...
          <div className="hidden sm:block bg-white rounded">
            <h2 className="mb-4 text-sm font-semibold text-gray-800 text-left">
              Formations
            </h2>
                    ...
                    >
                      {idx === 0 && (
                        <td
                          ...
                          className="px-4 py-2 align-top text-left"
                        >
                          {group.year}
                        </td>
                      )}
                      ...
                      <td className="px-4 py-2 text-left border-l border-r">
                        <SpecialityDesignation
                          id={f.formation_specialities.id_pract_speciality}
                        />
                      </td>
                      {idx === 0 && (
                        <td
                          rowSpan={group.entries.length}
                          className="px-4 py-2 align-top text-left border-r"
                        >
                          {group.etablissement}
                        </td>
                      )}
                  ...
                  <div className="p-2 text-xs">
                    {group.entries.map((f) => (
                      <div key={f.id_formation} className="mb-2 border-b pb-2">
                        <div className="mb-1">
                          <span className="font-semibold text-[#5DA781]">
                            Spécialité :
                          </span>
                          <SpecialityDesignation
                            id={f.formation_specialities?.id_pract_speciality}
