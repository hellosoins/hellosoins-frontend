...
const SignInForm = ({ onAccountCreated, isLoading }) => {
  ...
  const totalRequiredFields =
    requiredFieldsStep1.length + requiredFieldsStep2.length;
          ...
          <div className="hidden md:flex items-center pt-2 gap-4 justify-center">
            {steps.map(({ id, title }) => (
              <div key={id} className="flex items-center gap-2">
                <div
                  className={cn(
                    "border w-7 h-7 flex items-center justify-center rounded-full text-sm font-bold",
                    step >= id
                      ? "border-helloSoin text-helloSoin"
                      : "border-helloGray text-helloGray"
                  )}
                >
                  {id}
                </div>
                <span
                  className={
                    step >= id
                      ? "text-helloSoin text-xs"
                      : "text-helloGray text-xs"
                  }
                >
                  {title}
                </span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <Input
                      {...register("mail", {
                        ...
                        pattern: {
                          value: /^[^\s@]+@[^\s@]+\.[^\s@]+$/,
                          message:
                            "Veuillez entrer un email au format exemple@exemple.com",
                        },
                  ...
                  <div className="flex flex-col">
                    ...
                    <Controller
                      ...
                      rules={{
                        required:
                          "Veuillez renseigner votre numéro de téléphone mobile",
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="flex flex-col text-xs">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setValue("devise", e.target.value, {
                          shouldValidate: true,
                        })
                      }
                      value={watch("devise") || ""}
                  ...
                  <div className="flex flex-col text-xs">
                    ...
                    <select
                      {...register("echence", {
                        required:
                          "Veuillez renseigner votre spécialité principale",
                      })}
                      onChange={(e) =>
                        setValue("echence", e.target.value, {
                          shouldValidate: true,
                        })
                      }
                      value={watch("echence") || ""}
              ...
              <div className="flex flex-col space-y-6">
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  <Button
                    onClick={prevStep}
                    type="button"
                    className="w-full sm:w-[300px] border-2 border-helloBlue bg-white text-helloBlue hover:bg-helloBlue hover:text-white rounded-full text-xs"
                  >
                    Retour
                  </Button>
                  <Button
